...
export default {
	async fetch(request, env, ctx) {
		...
		if (env.UA) blockCrawlersUA = blockCrawlersUA.concat(await ADD(env.UA));
		...
		if (blockCrawlersUA.some(fxxk => userAgent.includes(fxxk)) && blockCrawlersUA.length > 0) {
			// 首页改成一个nginx伪装页
